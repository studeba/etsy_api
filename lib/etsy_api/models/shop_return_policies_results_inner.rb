...
module EtsyApi
  class ShopReturnPoliciesResultsInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EtsyApi::ShopReturnPoliciesResultsInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EtsyApi::ShopReturnPoliciesResultsInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'accepts_returns')
        self.accepts_returns = attributes[:'accepts_returns']
      end

      if attributes.key?(:'accepts_exchanges')
        self.accepts_exchanges = attributes[:'accepts_exchanges']
      end

      if attributes.key?(:'return_deadline')
        self.return_deadline = attributes[:'return_deadline']
      end
    end
    ...
    def hash
      [return_policy_id, shop_id, accepts_returns, accepts_exchanges, return_deadline].hash
    end
