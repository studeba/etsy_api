...
module EtsyApi
  class UpdateVariationImagesRequestVariationImagesInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EtsyApi::UpdateVariationImagesRequestVariationImagesInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EtsyApi::UpdateVariationImagesRequestVariationImagesInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @value_id.nil?
        invalid_properties.push('invalid value for "value_id", value_id cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @value_id.nil?
      return false if @value_id < 1
      return false if @image_id.nil?
      return false if @image_id < 1
      true
    end
    ...
    def value_id=(value_id)
      if value_id.nil?
        fail ArgumentError, 'value_id cannot be nil'
      end
    ...
    def hash
      [property_id, value_id, image_id].hash
    end
