...
module EtsyApi
  class UpdateListingInventoryRequestProductsInnerPropertyValuesInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `EtsyApi::UpdateListingInventoryRequestProductsInnerPropertyValuesInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `EtsyApi::UpdateListingInventoryRequestProductsInnerPropertyValuesInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'value_ids')
        if (value = attributes[:'value_ids']).is_a?(Array)
          self.value_ids = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @value_ids.nil?
        invalid_properties.push('invalid value for "value_ids", value_ids cannot be nil.')
      end

      if !@scale_id.nil? && @scale_id < 1
        invalid_properties.push('invalid value for "scale_id", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @property_id < 1
      return false if @value_ids.nil?
      return false if !@scale_id.nil? && @scale_id < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] scale_id Value to be assigned
    def scale_id=(scale_id)
      if !scale_id.nil? && scale_id < 1
        fail ArgumentError, 'invalid value for "scale_id", must be greater than or equal to 1.'
      end
    ...
    def hash
      [property_id, value_ids, scale_id, property_name, values].hash
    end
